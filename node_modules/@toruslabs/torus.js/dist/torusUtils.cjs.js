...
__webpack_require__.d(__webpack_exports__, {
  ...
  legacyKeyAssign: () => (/* reexport */ legacyKeyAssign),
  legacyKeyLookup: () => (/* reexport */ legacyKeyLookup),
  legacyWaitKeyLookup: () => (/* reexport */ legacyWaitKeyLookup),
...
;// CONCATENATED MODULE: ./src/helpers/common.ts
...
;// CONCATENATED MODULE: ./src/helpers/errorUtils.ts
...
;// CONCATENATED MODULE: ./src/helpers/keyUtils.ts
...
;// CONCATENATED MODULE: ./src/Share.ts
...
;// CONCATENATED MODULE: ./src/Polynomial.ts
...
;// CONCATENATED MODULE: ./src/helpers/langrangeInterpolatePoly.ts
...
function generateRandomPolynomial(ecCurve, degree, secret, deterministicShares) {
  let actualS = secret;
  if (!secret) {
    actualS = generatePrivateExcludingIndexes([new (external_bn_js_default())(0)]);
  }
  ...
  for (let i = 0; i < degree - deterministicShares.length; i += 1) {
    let shareIndex = generatePrivateExcludingIndexes([new (external_bn_js_default())(0)]);
    while (points[shareIndex.toString("hex", 64)] !== undefined) {
      shareIndex = generatePrivateExcludingIndexes([new (external_bn_js_default())(0)]);
    }
    points[shareIndex.toString("hex", 64)] = new src_Point(shareIndex, new (external_bn_js_default())((0,eccrypto_namespaceObject.generatePrivate)()), ecCurve);
  }
...
;// CONCATENATED MODULE: ./src/helpers/metadataUtils.ts
...
async function getOrSetNonce(legacyMetadataHost, ecCurve, serverTimeOffset, X, Y, privKey) {
  ...
  return (0,http_helpers_namespaceObject.post)(`${legacyMetadataHost}/get_or_set_nonce`, data, undefined, {
    useAPIKey: true
  });
}
...
;// CONCATENATED MODULE: ./src/helpers/nodeUtils.ts
...
async function retrieveOrImportShare(params) {
  ...
  for (let i = 0; i < endpoints.length; i += 1) {
    ...
    const p = (0,http_helpers_namespaceObject.post)(endpoints[i], (0,http_helpers_namespaceObject.generateJsonRPCObject)(JRPC_METHODS.COMMITMENT_REQUEST, {
      ...
      loglevel.error("commitment error", err);
    });
    promiseArr.push(p);
  }
  ...
  return Some(promiseArr, resultArr => {
    ...
    for (let i = 0; i < endpoints.length; i += 1) {
      ...
      if (isImportShareReq) {
        ...
        const p = (0,http_helpers_namespaceObject.post)(endpoints[i], (0,http_helpers_namespaceObject.generateJsonRPCObject)(JRPC_METHODS.IMPORT_SHARE, {
          encrypted: "yes",
          use_temp: true,
    ...
    return Some(promiseArrRequest, async (shareResponses, sharedState) => {
      ...
      if (!thresholdNonceData && !verifierParams.extended_verifier_id && !external_toruslabs_constants_namespaceObject.LEGACY_NETWORKS_ROUTE_MAP[network]) {
      ...
      if (completedRequests.length >= thresholdReqCount && thresholdPublicKey && (thresholdNonceData || verifierParams.extended_verifier_id || external_toruslabs_constants_namespaceObject.LEGACY_NETWORKS_ROUTE_MAP[network])) {
        const sharePromises = [];
        ...
        for (let i = 0; i < completedRequests.length; i += 1) {
          ...
          if ((sessionTokenSigs === null || sessionTokenSigs === void 0 ? void 0 : sessionTokenSigs.length) > 0) {
            ...
            if (sessionTokenSigMetadata && (_sessionTokenSigMetad = sessionTokenSigMetadata[0]) !== null && _sessionTokenSigMetad !== void 0 && _sessionTokenSigMetad.ephemPublicKey) {
              sessionTokenSigPromises.push(decryptNodeData(sessionTokenSigMetadata[0], sessionTokenSigs[0], sessionAuthKey).catch(err => loglevel.debug("session sig decryption", err)));
            } else {
          ...
          if ((sessionTokens === null || sessionTokens === void 0 ? void 0 : sessionTokens.length) > 0) {
            ...
            if (sessionTokenMetadata && (_sessionTokenMetadata = sessionTokenMetadata[0]) !== null && _sessionTokenMetadata !== void 0 && _sessionTokenMetadata.ephemPublicKey) {
              sessionTokenPromises.push(decryptNodeData(sessionTokenMetadata[0], sessionTokens[0], sessionAuthKey).catch(err => loglevel.debug("session token sig decryption", err)));
            } else {
          ...
          if ((keys === null || keys === void 0 ? void 0 : keys.length) > 0) {
            ...
            if (latestKey.share_metadata) {
              sharePromises.push(decryptNodeData(latestKey.share_metadata, Buffer.from(latestKey.share, "base64").toString("binary").padStart(64, "0"), sessionAuthKey).catch(err => loglevel.debug("share decryption", err)));
            }
          } else {
    ...
    const oAuthPubKey = (0,eccrypto_namespaceObject.getPublic)(Buffer.from(oAuthKey.toString(16, 64), "hex")).toString("hex");
...
const legacyKeyAssign = async _ref => {
  ...
  try {
    ...
    return await (0,http_helpers_namespaceObject.post)(endpoints[nodeNum], objectSpread2_default()(objectSpread2_default()({}, data), signedData), {
      headers: {
...
class Torus {
  constructor(_ref) {
    ...
    this.signerHost = `${external_toruslabs_constants_namespaceObject.SIGNER_MAP[network]}/api/sign`;
  }
  get isLegacyNetwork() {
    const legacyNetwork = external_toruslabs_constants_namespaceObject.LEGACY_NETWORKS_ROUTE_MAP[this.network];
  ...
  async importPrivateKey(endpoints, nodeIndexes, nodePubkeys, verifier, verifierParams, idToken, newPrivateKey) {
    ...
    const randomNonce = new (external_bn_js_default())((0,eccrypto_namespaceObject.generatePrivate)());
    ...
    for (let i = 0; i < nodeIndexesBn.length; i++) {
      ...
      encPromises.push((0,eccrypto_namespaceObject.encrypt)(Buffer.from(nodePubKey.getPublic().encodeCompressed("hex"), "hex"), Buffer.from(shareJson.share, "hex")));
    }
          ...
          for (let i = 0; i < shareResponses.length; i += 1) {
            ...
            if ((currentShareResponse === null || currentShareResponse === void 0 || (_currentShareResponse = currentShareResponse.result) === null || _currentShareResponse === void 0 || (_currentShareResponse = _currentShareResponse.keys) === null || _currentShareResponse === void 0 ? void 0 : _currentShareResponse.length) > 0) {
              ...
              if (firstKey.Metadata) {
                ...
                sharePromises.push((0,eccrypto_namespaceObject.decrypt)(tmpKey, objectSpread2_default()(objectSpread2_default()({}, metadata), {}, {
  ...
  async getNewPublicAddress(endpoints, _ref5, enableOneKey) {
    ...
    loglevel.debug("> torus.js/getPublicAddress, oAuthKeyData", {
      X: oAuthX,
      Y: oAuthY,
      oAuthAddress,
